...
@synthesize bookmarkDesc;
@synthesize oldBookmarkFolder;
@synthesize oldBookmarkSettings;
...
- (void)keyboardWillShow:(NSNotification *)notif
{
	...
	{
		inputTable.frame = CGRectMake(0, 44, 480, 106);
	}
}
...
- (void)keyboardWillHide:(NSNotification *)notif
{
	...
	if(isInLandscapeMode)
	{
		inputTable.frame = CGRectMake(0, 44, 480, 276);
	}
}
...
- (void)viewDidLoad {
	...
	bookmarkName = [oldBookmarkSettings objectForKey:kIBrowserLinkName];
	bookmarkLink = [oldBookmarkSettings objectForKey:kIBrowserLinkURL];
	bookmarkDesc = [oldBookmarkSettings objectForKey:kIBrowserLinkDesc];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	{	
		...
		else if([indexPath row] == 2)
		{
			...
			bookmarkDescTextField.text = bookmarkDesc;
...
- (void) deleteOldBookmark {
	NSArray* bookmarkLinks = [bookmarkFoldersDict objectForKey:oldBookmarkFolder];
	
	NSMutableArray* newBookmarkLinks = [[NSMutableArray alloc] initWithCapacity:[bookmarkLinks count]];
	[newBookmarkLinks addObjectsFromArray:bookmarkLinks];
	[newBookmarkLinks removeObject:oldBookmarkSettings];
	
	[bookmarkFoldersDict setObject:newBookmarkLinks forKey:oldBookmarkFolder];
...
- (IBAction) saveAction: (id) sender {
	...
	[self deleteOldBookmark];
	
	[self loadBookmarks];
	
	bookmarkLinks = [bookmarkFoldersDict objectForKey:bookmarkFolder];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(indexPath.section == 1)
	{
		...
		folderController.editBookmarkController = self;
