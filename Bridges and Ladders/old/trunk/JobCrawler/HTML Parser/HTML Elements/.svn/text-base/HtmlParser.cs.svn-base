    ...
    internal class HtmlParser
    {
        ...
        private StringCollection GetTokens( string input )
        {
            ...
            while ( i < input.Length )
            {
                ...
                {
                    if (i + 3 < input.Length && input.Substring(i, 4).Equals("<!--"))
                    {
                        i += 4;
                        status = ParseStatus.ReadComment;                        
                    }
                    else if (i + 1 < input.Length && input.Substring(i, 2).Equals("<!"))
                    {
                        i += 2;
                        status = ParseStatus.ReadSGMLComment;
                    }
                    else if ( i + 1 < input.Length && input.Substring( i, 2 ).Equals( "</" ) )
                    {
                        i += 2;
                ...
                else if (status == ParseStatus.ReadComment)
                {
                    //we currently filter out comments
                    while (i + 2 < input.Length && !input.Substring(i, 3).Equals("-->"))
                    {
                        ++i;
                    }
                ...
                else if (status == ParseStatus.ReadSGMLComment)
                {
                    //we currently filter out comments
                ...
                else if (status == ParseStatus.ReadAttributeValue)
                {
                    ...
                    if (i < input.Length && input.Substring(i, 1).Equals("\""))
                    {
                        ...
                        //error check follows: if user has typed two closing quotes by mistake
                        if (i < input.Length && input.Substring(i, 1).Equals("\""))
                        {
                            ++i;
                        }
